...
| 路径 | 说明 |
|------|------|
| `tutorials/` | 教程 Notebook 与脚本（`tutorial1_data_definition.py` 数据定义、`tutorial2_operations_definition.py` 指令定义） |
...
```bash
cd tutorials
python3 tutorial1_data_definition.py       # Tutorial 1：Im2Row、填充、分块（控制台验证，不写全量 bin）
python3 tutorial2_operations_definition.py # Tutorial 2：UOP/insn 演示 + 写出 uop.bin、instructions.bin
```

`tutorial2_operations_definition.py` 文件头有**完整的模拟器执行步骤**（含 TSIM / FSIM 与 `compiler_output` 文件对照表）。
...
**仅运行 `tutorials/tutorial2_operations_definition.py` 时**只会生成 `uop.bin` 与 `instructions.bin`，不足以直接跑 FSIM；还需数据类二进制与 CSV（见 `tutorials/tutorial2_operations_definition.py` 第六节，或从 `examples_compute/lenet5_layer1/` 同步）。
...
cp compiler_output/uop.bin compiler_output/instructions.bin   # tutorial2_operations_definition.py 已生成则可省略
...
### LeNet-5 第一层（与 `tutorial2_operations_definition.py` / `insn_lenet5_layer1.py` 对齐）
...
|------|------|-----------|----------------------|
| `instructions.bin` | `operations_definition` / `tutorial2_operations_definition.py` | 需要 | 需要 |
| `uop.bin` | 同上 | 需要 | 需要 |
...
若目录中已有整网 ONNX 编译产物（`QLinearConv*.bin`），与 `tutorials/tutorial2_operations_definition.py` 无关；该脚本只会覆盖同目录下的 `uop.bin` 与 `instructions.bin`。
...
**Q：只跑了 `python3 tutorial2_operations_definition.py`，仿真报错缺文件？**  
A：正常。请按上文「FSIM 单层示例」或 `tutorials/tutorial2_operations_definition.py` 第六节补齐 `input.bin` 等，或改用 TSIM 的 `ComputeApp_lenet5_layer1` 路径。
