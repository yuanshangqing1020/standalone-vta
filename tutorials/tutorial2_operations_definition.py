...
一、与 Tutorial 1 的关系
================================================================================
Tutorial 1（tutorial1_data_definition.py）解决的是：
...
================================================================================
  ...
  python3 tutorial2_operations_definition.py
...
六、用模拟器执行（端到端步骤）
================================================================================
重要：仅执行 tutorial2_operations_definition.py 只会得到「指令 + UOP」两个文件。模拟器还需要「数据」二进制：
...
说明：
  - lenet5_layer1/ 目录内已附带 input.bin、weight.bin、accumulator.bin、
    out.bin、expected_out_sram.bin、memory_addresses.csv，无需再由 tutorial2_operations_definition.py 生成。
...
与 tutorial2_operations_definition.py 直接配套的「单层」跑法：先把 examples_compute/lenet5_layer1/ 中
...
（单层教程后缀为空字符串时，文件名为 input.bin、uop.bin 等。）
  ...
  cp compiler_output/uop.bin compiler_output/instructions.bin  # 已由 tutorial2_operations_definition.py 生成
...
与仅运行 tutorials/tutorial2_operations_definition.py 的路径不同。

--------------------------------------------------------------------------------
6.3 产物与模拟器输入对照表
--------------------------------------------------------------------------------
  文件                      | tutorial2_operations_definition.py 是否生成 | TSIM (lenet5_layer1) | FSIM (单层)
  --------------------------|---------------|----------------------|-------------
  uop.bin                   | 是            | 需要                 | 需要
...
--------------------------------------------------------------------------------
  ...
  A: 那是 examples/make nn_compiler 整网编译产物，与 tutorials/tutorial2_operations_definition.py 无关；
     tutorial2_operations_definition.py 只会覆盖同目录下的 uop.bin 与 instructions.bin。
